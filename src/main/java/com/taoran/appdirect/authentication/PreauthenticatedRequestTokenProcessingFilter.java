...
/**
 * Copied from internet for demo, this supports 2-leg authentication.
 * 
 * @author Ran
 *
 */
public class PreauthenticatedRequestTokenProcessingFilter extends UnauthenticatedRequestTokenProcessingFilter {
  ...
  protected void onValidSignature(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException {
    ...
    // to obtain an authenticated token (see
    ...
    StringBuilder responseValue = new StringBuilder(OAuthProviderParameter.oauth_token.toString()).append('=')
        .append(OAuthCodec.oauthEncode(tokenValue)).append('&').append(OAuthProviderParameter.oauth_token_secret.toString()).append('=')
